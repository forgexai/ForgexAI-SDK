...
class CrossmintExample {
  ...
  async demonstrateNFTMinting() {
    ...
    try {
      ...
      if (this.isWalletConnected) {
        ...
        const collection = await this.sdk.crossmint.createCollection(
          collectionConfig
        );
      ...
      console.log(
        ...
        JSON.stringify(batchNFTs[0], null, 2)
      );

      if (this.isWalletConnected) {
        const batchResult = await this.sdk.crossmint.batchMintNFTs(batchNFTs);
        console.log(
          `✅ Batch minted: ${batchResult?.transactionIds?.length || 0} NFTs`
        );
      }
    } catch (error) {
  ...
  async demonstrateCrossChainFunctionality() {
    ...
    try {
      ...
      console.log(
        `NFT will be available on ${crossChainConfig.targetChains.length} networks`
      );

      if (this.isWalletConnected) {
        const crossChainResult = await this.sdk.crossmint.mintCrossChain(
          crossChainConfig
        );
        console.log(
          `✅ Cross-chain mint initiated: ${
            crossChainResult?.status || "Success"
          }`
        );
      }
      ...
      if (this.isWalletConnected) {
        ...
        console.log(
          `✅ Bridge transaction: ${bridgeResult?.transactionId || "Success"}`
        );
      }
    } catch (error) {
  ...
  async demonstrateWalletIntegration() {
    ...
    try {
      ...
      if (this.isWalletConnected) {
        const emailWallet = await this.sdk.crossmint.createEmailWallet(
          emailWalletConfig
        );
        console.log(
          `✅ Email wallet created: ${emailWallet?.address || "Success"}`
        );
      }
  ...
  async demonstratePaymentProcessing() {
    ...
    try {
      ...
      console.log(
        `Payment Methods: ${checkoutConfig.paymentMethods.join(", ")}`
      );

      if (this.isWalletConnected) {
        const checkout = await this.sdk.crossmint.createCheckout(
          checkoutConfig
        );
      ...
      console.log(
        `Price: $${subscriptionConfig.price}/${subscriptionConfig.interval}`
      );
  ...
  async demonstrateMarketplaceIntegration() {
    ...
    try {
      ...
      console.log(
        `Starting Price: ${auctionConfig.startingPrice} ${auctionConfig.currency}`
      );
      console.log(
        `Reserve Price: ${auctionConfig.reservePrice} ${auctionConfig.currency}`
      );
      ...
      console.log(
        `Total Volume: ${salesMetrics.totalVolume} ${salesMetrics.currency}`
      );
      console.log(
        `Average Price: ${salesMetrics.averagePrice} ${salesMetrics.currency}`
      );
      console.log(
        `Highest Sale: ${salesMetrics.topSale} ${salesMetrics.currency}`
      );
      ...
      console.log(
        `Enforce Royalties: ${royaltyConfig.enforceRoyalties ? "Yes" : "No"}`
      );
